export const coliving = require('./coliving-1.png');
export const bestSpace = require('./best-space-2.png');
export const roommate = require('./roommate-3.png');
export const firstScreenBg = require('./first-screen-bg.png');
export const secondScreenBg = require('./second-screen-bg.png');
export const thirdScreenBg = require('./third-screen-bg.png');
export const getStartedBg = require('./get-started-bg.png');
export const rocketWelcome = require('./rocket-welcome.png');
export const google = require('./google-icon.png');
export const facebook = require('./facebook.png');
export const twitter = require('./twitter.png');
export const userIntroBG = require('./user_intro.png');
export const host = require('./host.png');
export const roomie = require('./roomie.png');
export const profileGroup = require('./profile-group.png');
export const house = require('./house.png');
export const moneyBag = require('./money-bag.png');
export const serviceGroup = require('./service-group.png');
export const bills = require('./bills.png');
export const rocket = require('./rocket.png');
export const flash = require('./flash.png');
export const netflix = require('./netflix.png');
export const note = require('./note.png');
export const user = require('./user.png');
